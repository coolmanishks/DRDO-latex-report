...
\geometry{a4paper,left = 25mm,top=35mm,right = 24mm,bottom=10mm} % or letterpaper (US) or a5paper or....
...
\lhead{}\chead{}\rhead{Project Report\textbf{\textbar DRDO}}
\lfoot{Programmable Singnal Processor Data Logger,Data Merger and Analysis}\cfoot{}\rfoot{\thepage}
\topmargin = -23pt
\headsep = 10pt
...
\allsectionsfont{\normalfont\sffamily\mdseries\upshape\bfseries}
...
\date{}
\date{\vspace{-30pt}}
       \renewcommand{\abstractname}{\LARGE\bfseries{Abstract\vspace{1.5cm}}}
...
\pagenumbering{roman}
\thispagestyle{plain}
\section*{\LARGE\bfseries{\centering{About DRDO}\\ }}
\addcontentsline{toc}{section}{\bfseries{About DRDO}}
...
\newpage
\thispagestyle{plain}
\section*{\LARGE\bfseries{\centering{Declaration}\\ }}
...
\newpage
\thispagestyle{plain}
\section*{\LARGE\bfseries{\centering{Acknowledgement}\\ }}
\addcontentsline{toc}{section}{\bfseries{Acknowledgement}}
...
\noindent Design and Development of Radar Systems are :
\begin{enumerate}
\item 
\textbf{Army}
\begin{itemize}
...
\subsection{RADAR BLOCK DIAGRAM}
 The block diagram given below Figure 4 shows the main components of the basic radar system and their operations are:
\begin{itemize}
...
\noindent There are many advantage of database management system; some of them are as follows.
 \begin{itemize}
...
\noindent Some disadvantages of DBMS are as follow:
\begin{itemize}
...
%chapter 6
\title{\textbf{\huge {CHAPTER 6}}}
\maketitle
\section{SOLUTION}

\subsection{DATA RESOURCE}
\noindent The data used in this project was raw binary radar data which was collected by simulation of radar in DRDO Laboratory. 
...
\begin{figure}[H]
   \includegraphics[width=0.85\linewidth]{psploggerblockdiagram.jpg}
...
\begin{figure}[H]
\centering
  \includegraphics[width=0.6\linewidth]{pspmergerblockdiagram.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{pspmergerflow.jpg}
  \caption{PSP Data Merger Data Flow Diagram}
  \label{fig:figure 20}
\end{figure}
\subsubsection{ANALYSIS USING MATLAB}
For analysis of data, MATLAB is used. The database containing table dwelldata is connected to matlab through ODBC driver. Then the data is imported from MATLAB and the data is stored in a cell array.
...
\begin{figure}[H]
\centering
 \includegraphics[width=0.9\linewidth]{MATLABflow.jpg}
  \caption{Analysis phase Data Flow Diagram}
  \label{fig:figure 21}
\end{figure}

%chapter 7
\title{\textbf{\huge {CHAPTER 7}}}
\maketitle
\section{RESULT OBTAINED}

\subsection{RESULTS}
\noindent The results were obtained as per the aspectation and the software successfully logged and merged radar data which was further used for analysis. 
...
\includegraphics[width=\linewidth]{azimuth(20000,20100).jpg}
  ...
  \noindent \textbf{Definition : }\\
   ...
   \begin{figure}[H]
   \centering
 \includegraphics[width=0.6\linewidth]{elevation.jpg}
  \caption{Elevation is as shown in the figure}
  \label{fig:figure 35}
  \end{figure}
  \noindent \textbf{Plots : }The dwellcount and elevation column is stored in an array variable x and y and plots where made as shown in Figure:
  
  \subsubsection{Plot for dwellcount 274 to 25344}
     \begin{figure}[H]
\includegraphics[width=\linewidth]{dwellcount-elevation(1,25067).jpg}
...
The graph between dwellcount vs Elevation shows that the radar was operated priodically between 0.12 to - 0.14 elevation .
  ...
  \subsubsection{Plot for dwellcount 274 to 285}
  \begin{figure}[H]
\includegraphics[width=\linewidth]{elevation(1,10).jpg}
  \caption{Plot of dwellcount vs elevation graph of all the dwellcount from 274 to 25344}
  \label{fig:figure 37}
  \end{figure}
  \begin{tcolorbox}[title =\textbf{Analysis}]
  ...
  \subsubsection{Plot for dwellcount 4274 to 4295}
   \begin{figure}[H]
\includegraphics[width=\linewidth]{elevation(4000,4020).jpg}
  ...
  \end{tcolorbox}
   ...
   \subsubsection{Plot for dwellcount 10274 to 10325}
   \begin{figure}[H]
\includegraphics[width=\linewidth]{elevation(10000,10050).jpg}
  ...
  \end{tcolorbox}
   ...
   \subsubsection{Plot for dwellcount 20274 to 20375}
   \begin{figure}[H]
\includegraphics[width=\linewidth]{elevation(20000,20100).jpg}
  \caption{Plot of dwellcount vs elevation graph of all the dwellcount from 20274 to 20375}
  \label{fig:figure 40}
  \end{figure}
  \begin{tcolorbox}[title =\textbf{Analysis}]
  The graph  between dwellcount vs elevation from 20274 to 20375 dwellcout, shows many cycles of peaks and troughs which implies that the radar is continuously radiating dwells at increasing elevation and then come back at some particular lower elevation level and again starts radiating dwells at increasing elevation level and the peak is at a higher position than the previous peak.
  \end{tcolorbox}
  \begin{tcolorbox}[colframe=red]
    The overall analysis of dwellcount vs elevation graph shows that the radar is sending  dwells at increasing elevation and then decreasing it to a particular level and repeats the cycle by increasing the peak with each cycle of dwellcounts till 0.12 and then it sweeps back to -0.14 and starts the process again.
   \end{tcolorbox}
  
  %CHAPTER 9 
  \title{\textbf{\huge {CHAPTER 9}}}
\maketitle
\section{CONCLUSION AND FUTURE WORK}

\subsection{CONCLUSION}
The Project successfully logged 10 GB QRSAM raw radar binary files in MS SQL database and the application is successfully capable of merging clutter free binary radar data.
...
Analysis of data using MATLAB was successfully performed and various inferences and conclusions from the graphs were made. This project demonstrates the benefits of using
a database in the pre-signal processing of raw radar data files of large size and having large clutter data. The project was successful in its objective and need future work on processing of data in the database.
 
 \subsection{FUTURE WORK}
 The radar data further after logging in relational database can be used in future for many porposes like :
 \begin{itemize}
 ...
 \item Further the data can be used to get the information of radar's previous records.
 \item It can also be used to simulate, check and analyse the output of radar systems.
 \item It also open doors to process data and compare the efficiencies of different radar systems.
 \end{itemize}
 \newpage
